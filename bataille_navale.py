...
def ship_is_sunk(ship) : 
    return True if True not in ship.values() else False
#true si touche
def analize_shot(ship, shot_coord):
    hit = False
    sunk = False
...
while ships_list:
    ...
    for ship in ships_list:
        hit_status = analize_shot(ship, shot_coord)
        if hit_status['hit'] or hit_status['sunk']:
            break
    else:
